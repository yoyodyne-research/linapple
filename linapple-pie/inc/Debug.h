...
// Commands
extern const int NUM_COMMANDS_WITH_ALIASES; // = sizeof(g_aCommands) / sizeof
                                            // (Command_t); // Determined at
                                            // compile-time ;-)
...
extern ZeroPagePointers_t
    g_aZeroPagePointers[MAX_ZEROPAGE_POINTERS]; // TODO: use vector<> ?
...
// Breakpoints
bool GetBreakpointInfo(WORD nOffset, bool &bBreakpointActive_,
...
// Symbol Table / Memory
bool FindAddressFromSymbol(LPCSTR pSymbol, WORD *pAddress_ = NULL,
                           int *iTable_ = NULL);
WORD GetAddressFromSymbol(
    LPCTSTR symbol); // HACK: returns 0 if symbol not found
void SymbolUpdate(Symbols_e eSymbolTable, char *pSymbolName, WORD nAddrss,
                  bool bRemoveSymbol, bool bUpdateSymbol);
