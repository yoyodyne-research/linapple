...
// static const DWORD SAMPLE_RATE = 44100;	// Use a base freq so that
// DirectX (or sound h/w) doesn't have to up/down-sample
...
//
  ...
  //	HRESULT hr =
  //MockingboardVoice.lpDSBvoice->GetCurrentPosition(&dwCurrentPlayCursor,
  ...
  /*
          ...
          else
          {
                  ...
                  {
                          // |-----PxxxxxW-----|
                          if((dwByteOffset > dwCurrentPlayCursor) &&
     (dwByteOffset < dwCurrentWriteCursor))
                                  dwByteOffset = dwCurrentWriteCursor;
                  }
                  else
     ...
     (dwByteOffset < dwCurrentWriteCursor))
          ...
          if(nBytesRemaining < g_dwDSMockBufferSize / 4)
                  nNumSamplesError++;
     // < 0.25 of buffer remaining
          else if(nBytesRemaining > g_dwDSMockBufferSize / 2)
                  nNumSamplesError--;
     // > 0.50 of buffer remaining
          else
                  nNumSamplesError = 0;						//
     Acceptable amount of data in buffer*/
...
/*	if(!DSGetLock(MockingboardVoice.lpDSBvoice,
                                                dwByteOffset,
   (DWORD)nNumSamples*sizeof(short)*g_nMB_NumChannels,
                                                &pDSLockedBuffer0,
   ...
   &dwDSLockedBufferSize1))
        ...
        if(pDSLockedBuffer1)
                memcpy(pDSLockedBuffer1,
   &g_nMixBuffer[dwDSLockedBufferSize0/sizeof(short)], dwDSLockedBufferSize1);

        // Commit sound buffer
        hr = MockingboardVoice.lpDSBvoice->Unlock((void*)pDSLockedBuffer0,
   dwDSLockedBufferSize0,
                                                                                (void*)pDSLockedBuffer1, dwDSLockedBufferSize1);*/

//	dwByteOffset = (dwByteOffset +
//(DWORD)nNumSamples*sizeof(short)*g_nMB_NumChannels) % g_dwDSMockBufferSize;
...
void MB_UpdateCycles(ULONG uExecutedCycles) {
  ...
  for (int i = 0; i < NUM_SY6522; i++) {
    ...
    bool bTimer2Underflow =
        (!(OldTimer2 & 0x8000) && (pMB->sy6522.TIMER2_COUNTER.w & 0x8000));
