...
/*
        ...
        bit3	RESET_C2		SET_C2
   ( C2_OUTPUT & C2_SETMODE )
        bit3	STROBE_C1_RESET	STROBE_E_RESET		(
   C2_OUTPUT & C2_STROBE_MODE )
  ...
  case PIA_DDRA:
    ...
    if (OUTPUT_SELECTED(m_byCTLA)) {
      ...
      if (C2_OUTPUT(m_byCTLA) && C2_STROBE_MODE(m_byCTLA)) {
        // this will cause a transition low; call the output function if we're
        // currently high
        if (m_byOCA2)
...
void C6821::SetCA1(BYTE byData) {
  ...
  if (m_byCA1 ^ byData) {
    ...
    if ((byData && C1_LOW_TO_HIGH(m_byCTLA)) ||
      ...
      // CA2 is configured as output and in read strobe mode and cleared by a
      // CA1 transition
      if (C2_OUTPUT(m_byCTLA) && C2_STROBE_MODE(m_byCTLA) &&
          STROBE_C1_RESET(m_byCTLA)) {
...
void C6821::SetCB1(BYTE byData) {
  ...
  if (m_byCB1 ^ byData) {
    ...
    if ((byData && C1_LOW_TO_HIGH(m_byCTLB)) ||
      ...
      // CB2 is configured as output and in read strobe mode and cleared by a
      // CA1 transition
      if (C2_OUTPUT(m_byCTLB) && C2_STROBE_MODE(m_byCTLB) &&
          STROBE_C1_RESET(m_byCTLB)) {
...
void C6821::SetPB(BYTE byData) { m_byIB = byData; }

BYTE C6821::GetPA() { return m_byOA & m_byDDRA; }

BYTE C6821::GetPB() { return m_byOB & m_byDDRB; }
