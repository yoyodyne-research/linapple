...
TCHAR g_sFTPUserPass[512] =
    TEXT("anonymous:mymail@hotmail.com"); // full login line
...
void ContinueExecution() {
  ...
  const double fExecutionPeriodClks =
      g_fCurrentCLK6502 * ((double)nExecutionPeriodUsec / fUsecPerSec);

  bool bScrollLock_FullSpeed = g_bScrollLock_FullSpeed; // g_uScrollLockToggle;

  g_bFullSpeed =
      ((g_dwSpeed == SPEED_MAX) || bScrollLock_FullSpeed ||
  ...
  if (g_dwCyclesThisFrame >= dwClksPerFrame) {
    ...
    if (g_nAppMode != MODE_LOGO) {
      ...
      if ((!anyupdates) && (!lastupdates[0]) && (!lastupdates[1]) &&
        ...
        if ((!g_bFullSpeed) ||
            (currtime - lasttime >=
             (DWORD)((graphicsmode || !systemidle) ? 100 : 25))) {
          VideoRefreshScreen();
          lasttime = currtime;
        }
        screenupdated = 1;
...
int DoDiskInsert(int nDrive, LPSTR szFileName) {
  ...
  // 	}
  //
  // 	BOOL bWriteProtected = (dwAttributes & FILE_ATTRIBUTE_READONLY) ? TRUE :
  // FALSE;
...
void LoadConfiguration() {
  ...
  //  }
  // determine Apple type and set appropriate caption -- should be in
  // (F9)switching modes?
  switch (g_Apple2Type) {
  case A2TYPE_APPLE2:
  ...
  //       MB_SetVolume(dwTmp, 100);			// volume by default??
  //       --bb
  ...
  if (RegLoadString(TEXT("Configuration"), TEXT(REGVALUE_PPRINTER_FILENAME), 1,
                    &szHDFilename, MAX_PATH)) {
  ...
  // Define screen sizes
  if (RegLoadString(TEXT("Configuration"), TEXT("Screen factor"), 1,
                    &szFilename, 16)) {
  ...
  if (RegLoadString(TEXT("Configuration"), TEXT(REGVALUE_SAVESTATE_FILENAME), 1,
                    &szFilename, MAX_PATH)) {
    Snapshot_SetFilename(
        szFilename); // If not in Registry than default will be used
    free(szFilename);
    szFilename = NULL;
  }

  // Current/Starting Dir is the "root" of where the user keeps his disk images
  RegLoadString(TEXT("Preferences"), REGVALUE_PREF_START_DIR, 1, &szFilename,
                MAX_PATH);
  if (szFilename) {
  ...
  if (strlen(g_sCurrentDir) == 0 ||
      g_sCurrentDir[0] != '/')  // something is wrong in dir name?
  {                             //
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_PREF_HDD_START_DIR, 1,
                &szFilename, MAX_PATH);
  if (szFilename) {
  ...
  //  SetCurrentDirectory(g_sCurrentDir);
  if (strlen(g_sHDDDir) == 0 ||
      g_sHDDDir[0] != '/') // something is wrong in dir name?
  {
    char *tmp = getenv("HOME"); /* we don't have HOME?  ^_^  0_0  $_$  */
    if (tmp == NULL)
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_PREF_SAVESTATE_DIR, 1,
                &szFilename, MAX_PATH);
  if (szFilename) {
  ...
  if (strlen(g_sSaveStateDir) == 0 ||
      g_sSaveStateDir[0] != '/') // something is wrong in dir name?
  {
    char *tmp = getenv("HOME"); /* we don't have HOME?  ^_^  0_0  $_$  */
    if (tmp == NULL)
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_FTP_DIR, 1, &szFilename,
                MAX_PATH);
  if (szFilename) {
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_FTP_HDD_DIR, 1, &szFilename,
                MAX_PATH);
  if (szFilename) {
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_FTP_LOCAL_DIR, 1, &szFilename,
                MAX_PATH);
  if (szFilename) {
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_FTP_USERPASS, 1, &szFilename,
                512);
  if (szFilename) {
  ...
  //   RegLoadString(TEXT("Configuration"),TEXT("Uthernet
  //   Interface"),1,szUthernetInt,MAX_PATH);
  //   update_tfe_interface(szUthernetInt,NULL);
}

//===========================================================================
void RegisterExtensions() { // TO DO: register extensions for KDE or GNOME
                            // desktops?? Do not know, if it is sane idea.
                            // He-he. --bb
  ...
  // 	wsprintf(command, "\"%s\"",	szCommandTmp);	// Wrap	path & filename
  // in quotes &	null terminate
  //
  // 	TCHAR icon[MAX_PATH];
...
//{
// Sane idea: use getoptlong as command-line parameter preprocessor. Use it at
// your health. Ha. --bb
...
/*
        ...
        while(*lpCmdLine)
        {
                if(*lpCmdLine == '\"')
                {
                        ...
                        if(!bInQuotes)
                        {
                                *lpCmdLine++ = 0x00;	// Assume end-quote
   is end of this arg
                                continue;
                        }
                }
...
int main(int argc, char *lpCmdLine[]) {
  ...
  //		!strcmp(lpCmdLine[1],"-b"));	// if we should start benchmark (-b
...
/*
        while(*lpCmdLine)
        {
                ...
                {
                        g_fh = fopen("AppleWin.log", "a+t");	// Open log
file (append & text g_nAppMode)
...
// end of Unix(tm) specific code
                ...
                {
                        g_bDisableDirectSound = true; // without direct sound?
U-u-u-u-uuuuuuuhhhhhhhhh --bb
                }
#ifdef RAMWORKS
                else if(strcmp(lpCmdLine, "-r") == 0)		// RamWorks
size [1..127]
                {
                        lpCmdLine = lpNextArg;
                        lpNextArg = GetNextArg(lpCmdLine);
...
//
//             VerQueryValue(pVerInfoBlock, TEXT("\\"), (LPVOID*)
//             &pFixedFileInfo, (PUINT) &pFixedFileInfoLen);
//
//             // Construct version string from fixed file info block
//
//             unsigned long major     = pFixedFileInfo->dwFileVersionMS >> 16;
//             unsigned long minor     = pFixedFileInfo->dwFileVersionMS &
//             0xffff;
//             unsigned long fix       = pFixedFileInfo->dwFileVersionLS >> 16;
// 			unsigned long fix_minor = pFixedFileInfo->dwFileVersionLS &
// 0xffff;
//
//             sprintf(VERSIONSTRING, "%d.%d.%d.%d", major, minor, fix,
//             fix_minor);
//         }
//     }
  ...
  if (g_fh)
    fprintf(g_fh, "Performance frequency = %d\n", g_nPerfFreq);
#endif
  ...
  do {
    ...
    // 		if (!bSysClkOK)
    // 		{
    // 			MessageBox(g_hFrameWindow, "DirectX failed to create SystemClock
    // instance", TEXT("AppleWin Error"), MB_OK);
    // 			PostMessage(g_hFrameWindow, WM_DESTROY, 0, 0);	// Close everything
    // down
    // 		}
    ...
    // 		if(bSetFullScreen)
    // 		{
    // 			PostMessage(g_hFrameWindow, WM_KEYDOWN, VK_F1+BTN_FULLSCR,
    // 0);
    // 			PostMessage(g_hFrameWindow, WM_KEYUP,   VK_F1+BTN_FULLSCR,
    // 0);
    ...
    // 		if(bBoot)
    // 		{
    // 			PostMessage(g_hFrameWindow, WM_KEYDOWN, VK_F1+BTN_RUN,
    // 0);
    // 			PostMessage(g_hFrameWindow, WM_KEYUP,   VK_F1+BTN_RUN,
    // 0);
    // 			bBoot = false;
    // 		}
    ...
    CpuDestroy();
    MemDestroy();
    //		printf("Quitting. PrintDestroy, sg_SSC.CommDestroy,
    //CPU-MEMDestroy!\n");
    SpkrDestroy();
    ...
    sg_Mouse.Uninitialize(); // Maybe restarting due to switching slot-4 card
                             // from mouse to MB
