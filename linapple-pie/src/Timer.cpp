...
void SysClk_TickTimer(
    int signum) {        // should occur every specified times per second
  g_bTimerToggle = true; // just set the toggle flag, and leave peacefully? --bb
}

bool SysClk_InitTimer() { // first initialization of the timer
  /*	memset(&sa_SysClk, 0, sizeof(sa_SysClk));	// clear sigaction
     struct
...
void SysClk_StartTimerUsec(DWORD dwUsecPeriod) {
  ...
  // to comply with Windows DirectShow REFERENCE_TIME, which is in units of 100
  // nanoseconds
...
void SysClk_StartTimerUsec(DWORD dwUsecPeriod) {
  ...
  REFERENCE_TIME rtPeriod =
      (REFERENCE_TIME)(dwUsecPeriod * 10); // In units of 100ns
  ...
  if (g_pRefClock->AdvisePeriodic(rtNow, rtPeriod, g_hSemaphore,
                                  &g_dwAdviseToken) != S_OK) {
