...
void DiskInitialize() {
  ...
  /*	TCHAR imagefilename[MAX_PATH];
          ...
          _tcscat(imagefilename,TEXT("MASTER.DSK")); // TODO: Should remember
     last disk by user*/
...
DWORD DiskSetSnapshot(SS_CARD_DISK2 *pSS, DWORD /*dwSlot*/) {
  ...
  for (UINT i = 0; i < 2; i++) {
    ...
    // 			//dwAttributes =
    // GetFileAttributes(pSS->Unit[i].szFileName);
    // 		}
    //
    ...
    //
    // 			if(DiskInsert(i, pSS->Unit[i].szFileName, bWriteProtected,
    // 0))
    // 				bImageError = true;
    //
    // 			// DiskInsert() sets up:
