...
// (2) SSC recv IRQ example: http://www.wright.edu/~john.matthews/ssc.html#lst -
// John B. Matthews, 5/13/87
// (3) WaitCommEvent, etc:
// http://mail.python.org/pipermail/python-list/2002-November/131437.html
...
void CSuperSerialCard::UpdateCommState() {
  ...
  struct termios dcb;
  int l_databits = CS8;
  tcgetattr(m_hCommHandle,
            &dcb); // get current attributes for m_hCommHandle in dcb
  ...
  */
  ...
  //	dcb.c_cflag |= m_uBaudRate; // set here instead of by cfsetispeed and
  //cfsetospeed?
...
BYTE /*__stdcall*/ CSuperSerialCard::SSC_IORead(WORD PC, WORD uAddr,
                                                BYTE bWrite, BYTE uValue,
                                                ULONG nCyclesLeft) {
...
BYTE /*__stdcall*/ CSuperSerialCard::SSC_IOWrite(WORD PC, WORD uAddr,
                                                 BYTE bWrite, BYTE uValue,
                                                 ULONG nCyclesLeft) {
  UINT uSlot = ((uAddr & 0xff) >> 4) - 8;
  CSuperSerialCard *pSSC = (CSuperSerialCard *)MemGetSlotParameters(uSlot);
...
BYTE /*__stdcall*/ CSuperSerialCard::CommCommand(WORD, WORD, BYTE write,
                                                 BYTE value, ULONG) {
  if (!CheckComm())
    return 0;
  ...
  if (write && (value != m_uCommandByte)) {
    ...
    if (m_uCommandByte & 0x01) // Data Terminal Ready (DTR) setting [0=set DTR
                               // high (indicates 'not ready')]
    {
      // Note that, although the DTR is generally not used in the SSC (it may
      // actually not
      // be connected!), it must be set to 'low' in order for the 6551 to
      // function correctly.
    }
...
BYTE /*__stdcall*/ CSuperSerialCard::CommControl(WORD, WORD, BYTE write,
                                                 BYTE value, ULONG) {
  if (!CheckComm())
    return 0;
...
BYTE /*__stdcall*/ CSuperSerialCard::CommReceive(WORD, WORD, BYTE, BYTE,
                                                 ULONG) {
  if (!CheckComm())
    return 0;
...
BYTE /*__stdcall*/ CSuperSerialCard::CommTransmit(WORD, WORD, BYTE, BYTE value,
                                                  ULONG) {
  if (!CheckComm())
    return 0;
...
// 5    0       Data Carrier Detect (DCD) true [0=DCD low (detected), 1=DCD high
// (not detected)]
// 6    0       Data Set Ready (DSR) true [0=DSR low (ready), 1=DSR high (not
// ready)]
...
BYTE /*__stdcall*/ CSuperSerialCard::CommStatus(WORD, WORD, BYTE, BYTE, ULONG) {
...
#ifdef SUPPORT_MODEM
  DWORD modemstatus = 0;
  GetCommModemStatus(m_hCommHandle,
                     &modemstatus); // Returns 0x30 = MS_DSR_ON|MS_CTS_ON
#endif

  //

  // TO DO - ST_TX_EMPTY:
  // . IRQs enabled  : set after WaitCommEvent has signaled that TX has
  // completed
  ...
  BYTE uStatus = ST_TX_EMPTY | (m_vRecvBytes ? ST_RX_FULL : 0x00)
                 ...
                 | ((modemstatus & MS_RLSD_ON)
                        ? 0x00
                        : ST_DCD) // Need 0x00 to allow ZLink to start up
...
BYTE /*__stdcall*/ CSuperSerialCard::CommDipSw(WORD, WORD addr, BYTE, BYTE,
                                               ULONG) {
  BYTE sw = 0;
  switch (addr & 0xf) {
  case 1: // DIPSW1
    sw = (BaudRateToIndex(m_DIPSWCurrent.uBaudRate) << 4) |
         m_DIPSWCurrent.eFirmwareMode;
    break;

  case 2: // DIPSW2
    // Comms mode - SSC manual, pg23/24
    BYTE INT = m_DIPSWCurrent.uStopBits == TWOSTOPBITS
                   ? 1
                   : 0; // SW2-1 (Stop bits: 1-ON(0); 2-OFF(1))
    BYTE DSR = 0;       // Always zero
    BYTE DCD = m_DIPSWCurrent.uByteSize == 7
                   ? 1
                   : 0; // SW2-2 (Data bits: 8-ON(0); 7-OFF(1))
    ...
    BYTE FE =
        m_DIPSWCurrent.bLinefeed ? 1 : 0; // SW2-5 (LF: yes-ON(0); no-OFF(1))
    BYTE PE = m_DIPSWCurrent.bInterrupts
                  ? 1
                  : 0; // SW2-6 (Interrupts: yes-ON(0); no-OFF(1))

    sw = (INT << 7) | (DSR << 6) | (DCD << 5) | (TDR << 4) | (RDR << 3) |
         (OVR << 2) | (FE << 1) | (PE << 0);
    break;
  }
  return sw;
...
void CSuperSerialCard::CommInitialize(LPBYTE pCxRomPeripheral, UINT uSlot) {
  ...
  /*	HRSRC hResInfo = FindResource(NULL, MAKEINTRESOURCE(IDR_SSC_FW),
     "FIRMWARE");
          if(hResInfo == NULL)
                  return;
  ...
  memcpy(pCxRomPeripheral + uSlot * 256, pData + SSC_SLOT_FW_OFFSET,
         SSC_SLOT_FW_SIZE);
  ...
  RegisterIoHandler(uSlot, &CSuperSerialCard::SSC_IORead,
                    &CSuperSerialCard::SSC_IOWrite, NULL, NULL, this,
                    m_pExpansionRom);
}
...
void CSuperSerialCard::CommUpdate(DWORD totalcycles) {
  ...
  if ((m_dwCommInactivity += totalcycles) > 1000000) {
    ...
    if ((m_dwCommInactivity > 2000000) ||
        (m_dwCommInactivity - lastcheck > 99950)) {
#ifdef SUPPORT_MODEM
      DWORD modemstatus = 0;
...
void CSuperSerialCard::CheckCommEvent(DWORD dwEvtMask) {
  ...
  {
    pthread_mutex_lock(&m_CriticalSection);
    //		ReadFile(m_hCommHandle, m_RecvBuffer, 1, (DWORD*)&m_vRecvBytes,
    //&m_o);
...
DWORD CSuperSerialCard::CommThread(LPVOID lpParameter) {
  ...
  //	BOOL bRes = SetCommMask(pSSC->m_hCommHandle, EV_TXEMPTY | EV_RXCHAR);
          BOOL bRes = SetCommMask(pSSC->m_hCommHandle, EV_RXCHAR);		//
  Just RX
          if (!bRes)
                  return -1;
  ...
  {pSSC->m_hCommEvent[COMMEVT_WAIT], pSSC->m_hCommEvent[COMMEVT_TERM]};
          HANDLE hCommEvent_Ack[nNumEvents]  = {pSSC->m_hCommEvent[COMMEVT_ACK],
  pSSC->m_hCommEvent[COMMEVT_TERM]};
  #else
          HANDLE hCommEvent_Wait[nNumEvents];
          ...
          while(1)
          {
                  ...
                  bRes = WaitCommEvent(pSSC->m_hCommHandle, &dwEvtMask,
  &pSSC->m_o);	// Will return immediately (probably with ERROR_IO_PENDING)
                  _ASSERT(!bRes);
                  if (!bRes)
                  {
                          ...
                          while(1)
                          {
                                  ...
                                  // On very 1st wait *only*: get a false signal
  (when not running via debugger)
                                  if ((dwWaitResult == WAIT_OBJECT_0) &&
  (dwEvtMask == 0))
                                          continue;

                                  if ((dwWaitResult >= WAIT_OBJECT_0) &&
  (dwWaitResult <= WAIT_OBJECT_0+nNumEvents-1))
                                          break;
                          }

                          dwWaitResult -= WAIT_OBJECT_0;			//
  Determine event # that signaled
...
bool CSuperSerialCard::CommThInit() {
  /*	_ASSERT(m_hCommThread == NULL);
          ...
          if ((m_hCommEvent[0] == NULL) && (m_hCommEvent[1] == NULL) &&
     (m_hCommEvent[2] == NULL))
          {
                  ...
                  m_hCommEvent[COMMEVT_ACK] = CreateEvent(NULL,	//
     lpEventAttributes
                                                                                  FALSE,	// bManualReset (FALSE = auto-reset)
                                                                                  FALSE,	// bInitialState (FALSE = non-signaled)
                                                                                  NULL);	// lpName
                  m_hCommEvent[COMMEVT_TERM] = CreateEvent(NULL,	//
     lpEventAttributes
                                                                                  FALSE,	// bManualReset (FALSE = auto-reset)
                                                                                  FALSE,	// bInitialState (FALSE = non-signaled)
                                                                                  NULL);	// lpName
     ...
     (m_hCommEvent[2] == NULL))
          ...
          if (m_hCommThread == NULL)
          {
                  ...
                  m_hCommThread = CreateThread(NULL,			//
     lpThreadAttributes
                                                                          0,
     // dwStackSize
                                                                          (LPTHREAD_START_ROUTINE)
     &CSuperSerialCard::CommThread,
                                                                          this,
     // lpParameter
                                                                          0,
...
void CSuperSerialCard::CommThUninit() {
  /*	if (m_hCommThread)
          {
                  SetEvent(m_hCommEvent[COMMEVT_TERM]);	// Signal to thread
     that it should exit
