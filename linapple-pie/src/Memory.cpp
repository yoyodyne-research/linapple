...
// static BOOL    fastpaging   = 0;	// Redundant: only ever set to 0, by
// MemSetFastPaging(0)
...
BYTE /*__stdcall*/ IO_Annunciator(WORD programcounter, WORD address, BYTE write,
                                  BYTE value, ULONG nCycles);
static void UpdatePaging(BOOL initialize, BOOL updatewriteonly);

//=============================================================================

static BYTE /*__stdcall*/ IORead_C00x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  return KeybReadData(pc, addr, bWrite, d, nCyclesLeft);
}

static BYTE /*__stdcall*/ IOWrite_C00x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  if ((addr & 0xf) <= 0xB)
...
static BYTE /*__stdcall*/ IORead_C01x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  switch (addr & 0xf) {
  case 0x0:
...
static BYTE /*__stdcall*/ IOWrite_C01x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  return KeybReadFlag(pc, addr, bWrite, d, nCyclesLeft);
}
...
static BYTE /*__stdcall*/ IORead_C02x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  return IO_Null(pc, addr, bWrite, d, nCyclesLeft);
}

static BYTE /*__stdcall*/ IOWrite_C02x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  return IO_Null(pc, addr, bWrite, d, nCyclesLeft);
}
...
static BYTE /*__stdcall*/ IORead_C03x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  return SpkrToggle(pc, addr, bWrite, d, nCyclesLeft);
}

static BYTE /*__stdcall*/ IOWrite_C03x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  return SpkrToggle(pc, addr, bWrite, d, nCyclesLeft);
}
...
static BYTE /*__stdcall*/ IORead_C04x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  return IO_Null(pc, addr, bWrite, d, nCyclesLeft);
}

static BYTE /*__stdcall*/ IOWrite_C04x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  return IO_Null(pc, addr, bWrite, d, nCyclesLeft);
}
...
static BYTE /*__stdcall*/ IORead_C05x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  switch (addr & 0xf) {
  case 0x0:
    return VideoSetMode(pc, addr, bWrite, d, nCyclesLeft);
...
static BYTE /*__stdcall*/ IOWrite_C05x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  switch (addr & 0xf) {
  case 0x0:
    return VideoSetMode(pc, addr, bWrite, d, nCyclesLeft);
...
static BYTE /*__stdcall*/ IORead_C06x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  switch (addr & 0xf) {
  case 0x0:
    return IO_Null(pc, addr, bWrite, d, nCyclesLeft);
...
static BYTE /*__stdcall*/ IOWrite_C06x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  return IO_Null(pc, addr, bWrite, d, nCyclesLeft);
}
...
static BYTE /*__stdcall*/ IORead_C07x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                      ULONG nCyclesLeft) {
  switch (addr & 0xf) {
  case 0x0:
...
static BYTE /*__stdcall*/ IOWrite_C07x(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                                       ULONG nCyclesLeft) {
  switch (addr & 0xf) {
  case 0x0:
    return JoyResetPosition(pc, addr, bWrite, d, nCyclesLeft);
...
BYTE /*__stdcall*/ IO_Null(WORD programcounter, WORD address, BYTE write,
                           BYTE value, ULONG nCyclesLeft) {
  if (!write)
...
BYTE /*__stdcall*/ IORead_Cxxx(WORD programcounter, WORD address, BYTE write,
                               BYTE value, ULONG nCyclesLeft) {
  if (address == 0xCFFF) {
...
BYTE /*__stdcall*/ IOWrite_Cxxx(WORD programcounter, WORD address, BYTE write,
                                BYTE value, ULONG nCyclesLeft) {
  return 0;
}
...
void RegisterIoHandler(UINT uSlot, iofunction IOReadC0, iofunction IOWriteC0,
                       iofunction IOReadCx, iofunction IOWriteCx,
...
static void UpdatePaging(BOOL initialize, BOOL updatewriteonly) {
  ...
  for (loop = 0x02; loop < 0xC0; loop++) {
    ...
    memwrite[loop] =
        ((SW_AUXREAD != 0) == (SW_AUXWRITE != 0))
            ? mem + (loop << 8)
            : SW_AUXWRITE ? memaux + (loop << 8) : memmain + (loop << 8);
  }
  ...
  for (loop = 0xE0; loop < 0x100; loop++) {
    memshadow[loop] =
        SW_HIGHRAM ? SW_ALTZP ? memaux + (loop << 8) : memmain + (loop << 8)
                   : memrom + ((loop - 0xD0) * 0x100);

    memwrite[loop] =
        SW_WRITERAM
            ? SW_HIGHRAM ? mem + (loop << 8) : SW_ALTZP ? memaux + (loop << 8)
                                                        : memmain + (loop << 8)
            : NULL;
  }
...
bool MemGet80Store() { return SW_80STORE != 0; }

//===========================================================================

bool MemCheckSLOTCXROM() { return SW_SLOTCXROM ? true : false; }
...
LPBYTE MemGetAuxPtr(WORD offset) {
  ...
  if (((SW_PAGE2 && SW_80STORE) || VideoGetSW80COL()) &&
      ((((offset & 0xFF00) >= 0x0400) && ((offset & 0xFF00) <= 0700)) ||
...
LPBYTE MemGetCxRomPeripheral() { return pCxRomPeripheral; }
...
int MemInitialize() // returns -1 if any eror during initialization
{
  ...
  //  // THE MEMIMAGE BUFFER CAN CONTAIN EITHER MULTIPLE MEMORY IMAGES OR ONE
  //  MEMORY IMAGE WITH COMPILER DATA
  //  memimage = (LPBYTE)VirtualAlloc(NULL,
  //                                  MAX(0x30000,MAXIMAGES*0x10000),
  //                                  MEM_RESERVE,PAGE_NOACCESS);
  memimage =
      (LPBYTE)VirtualAlloc(NULL, _6502_MEM_END + 1, /*MEM_RESERVE*/ MEM_COMMIT,
  ...
  pCxRomInternal =
      (LPBYTE)VirtualAlloc(NULL, CxRomSize, MEM_COMMIT, PAGE_READWRITE);
  pCxRomPeripheral =
      (LPBYTE)VirtualAlloc(NULL, CxRomSize, MEM_COMMIT, PAGE_READWRITE);
  ...
  if (!memaux || !memdirty || !memimage || !memmain || !memrom ||
    ...
    /*		MessageBox(
                            GetDesktopWindow(),
                            TEXT("The emulator was unable to allocate the memory
       it ")
                            TEXT("requires.  Further execution is not
       possible."),
                            g_pAppTitle,
...
//	LPVOID newloc =
//VirtualAlloc(memimage,0x30000,MEM_COMMIT,PAGE_READWRITE);
// 	LPVOID newloc =
// VirtualAlloc(memimage,_6502_MEM_END+1,MEM_COMMIT,PAGE_READWRITE);
// 	if (newloc != memimage)
// 		MessageBox(
// 			GetDesktopWindow(),
...
// 			MB_ICONEXCLAMATION | MB_SETFOREGROUND);
  ...
  while ((i < g_uMaxExPages) &&
         (RWpages[i] = (LPBYTE)VirtualAlloc(NULL, _6502_MEM_END + 1, MEM_COMMIT,
                                            PAGE_READWRITE)))
    i++;
#endif
...
#define IDR_APPLE2E_ENHANCED_ROM "Apple2e_Enhanced.rom"
  ...
  case A2TYPE_APPLE2EEHANCED:
    ...
    ROM_SIZE = Apple2eRomSize;
    break;
    /*case A2TYPE_PRAVEC8C:RomFileName = IDR_PRAVEC_8C; ROM_SIZE =
     * Apple2Pravec8CSize;break;*/
  }
  ...
  if (RomFileName == NULL) {
    /*		TCHAR sRomFileName[ MAX_PATH ];
                    switch (g_Apple2Type)
                    {
                    case A2TYPE_APPLE2:			_tcscpy(sRomFileName,
       TEXT("APPLE2.ROM")); break;
                    case A2TYPE_APPLE2PLUS:		_tcscpy(sRomFileName,
       TEXT("APPLE2_PLUS.ROM")); break;
                    case A2TYPE_APPLE2E:		_tcscpy(sRomFileName,
       TEXT("APPLE2E.ROM")); break;
                    case A2TYPE_APPLE2EEHANCED:	_tcscpy(sRomFileName,
       TEXT("APPLE2E_ENHANCED.ROM")); break;
                    }

                    TCHAR sText[ MAX_PATH ];
                    wsprintf( sText, TEXT("Unable to open the required firmware
       ROM data file.\n\nFile: %s"), sRomFileName );
  ...
  /*	void * BUFFER = malloc(ROM_SIZE);
          if(BUFFER == NULL) {
                  fprintf(stderr, "Unable to allocate %d bytes of memory for
     ROM.\n", ROM_SIZE);
                  return -1;
          }
          FILE * romfile;
     ...
     RomFileName, ROM_SIZE);
          ...
          fclose(romfile);
          if(nbytes != ROM_SIZE) {
                  fprintf(stderr, "Size of %s ROM file mismatches required %d
     bytes\n", RomFileName, ROM_SIZE);
                  free(BUFFER);
...
// . Snapshot_LoadState()
void MemResetPaging() { ResetPaging(0); }

//===========================================================================

// Called by Disk][ I/O only
BYTE MemReturnRandomData(BYTE highbit) {
  static const BYTE retval[16] = {0x00, 0x2D, 0x2D, 0x30, 0x30, 0x32,
                                  0x32, 0x34, 0x35, 0x39, 0x43, 0x43,
                                  0x43, 0x60, 0x7F, 0x7F};
...
//===========================================================================
BYTE /*__stdcall*/ MemSetPaging(WORD programcounter, WORD address, BYTE write,
                                BYTE value, ULONG nCyclesLeft) {
  address &= 0xFF;
