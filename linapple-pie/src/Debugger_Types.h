...
/*
        Legend:
                A = Absolute (fortunately Accumulator is implicit, leaving us to
   use 'A')
...
/*
        ...
        This allows one to set a bunch of breakpoints, and re-enable the ones
   you want
...
// NOTE: Commands_e and g_aCommands[] order _MUST_ match !!! Aliases are listed
// at the end
enum Commands_e {
  // CPU
  ...
  ,
  CMD_CONFIG_FONT
  //		, CMD_CONFIG_FONT2 // PARAM_FONT_DISASM PARAM_FONT_INFO
  //PARAM_FONT_SOURCE
  ,
...
Update_t CmdBenchmarkStart(int nArgs); // Update_t CmdSetupBenchmark (int
                                       // nArgs);
...
enum MemoryView_e {
  ...
  //		MEM_VIEW_TXT_LO, // Ctrl Chars mapped to visible range, and
  //inverse
...
/* i.e.
                SYM LOAD = $C600   (1) type: string, nVal1 = symlookup; (2)
   type: operator, token: EQUAL; (3) type: address, token:DOLLAR
                BP LOAD            type:
...
enum Parameters_e {
  ...
  //		, PARAM_CODE_1  // disasm top // removed, since can't set top window
  //for code/data
  ,
  ...
  PARAM_CONSOLE,
  PARAM_DATA // data all
  //		, PARAM_DATA_1  // data top // removed, since can't set top window
  //for code/data
  ,
  ...
  _PARAM_SRC // alias SRC = SOURCE
             //		, PARAM_SOURCE_1 // source top // removed, since can't set top
             //window for code/data
  ,
  PARAM_SOURCE_2 // source bot
  ,
  PARAM_SYMBOLS,
  _PARAM_SYM // alias SYM = SYMBOLS
             //		, PARAM_SYMBOL_1 // symbols top // removed, since can't set top
             //window for code/data
  ,
