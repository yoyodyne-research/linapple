...
CMouseInterface::~CMouseInterface() { delete[] m_pSlotRom; }
...
void CMouseInterface::Initialize(LPBYTE pCxRomPeripheral, UINT uSlot) {
  ...
  // 	HRSRC hResInfo = FindResource(NULL,
  // MAKEINTRESOURCE(IDR_MOUSEINTERFACE_FW), "FIRMWARE");
  // 	if(hResInfo == NULL)
  // 		return;
  //
  ...
  // instead of reading wndzooozes resources, just read it from a mere file on a
  // disk!? bb
  ...
  SetSlotRom();
  RegisterIoHandler(uSlot, &CMouseInterface::IORead, &CMouseInterface::IOWrite,
                    NULL, NULL, this, NULL);
  m_bActive = true;
...
BYTE CMouseInterface::IORead(WORD PC, WORD uAddr, BYTE bWrite, BYTE uValue,
                             ULONG nCyclesLeft) {
...
BYTE CMouseInterface::IOWrite(WORD PC, WORD uAddr, BYTE bWrite, BYTE uValue,
                              ULONG nCyclesLeft) {
  UINT uSlot = ((uAddr & 0xff) >> 4) - 8;
  CMouseInterface *pMouseIF = (CMouseInterface *)MemGetSlotParameters(uSlot);
...
void CMouseInterface::On6821_A(BYTE byData) { m_by6821A = byData; }
...
void CMouseInterface::OnMouseEvent() {
  ...
  byState &= ((m_byMode & 0x0E) | 0x20); // Keep "X/Y moved since last
                                         // READMOUSE" for next MOUSE_READ
                                         // (Contiki v1.3 uses this)
...
void CMouseInterface::SetPosition(int xvalue, int xrange, int yvalue,
                                  int yrange) {
