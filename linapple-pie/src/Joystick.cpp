...
// If Apple joystick #0 is using {KEYBOARD | MOUSE} then joystick #1 can't use
// it.
// If Apple joystick #1 is using KEYBOARD, then disable the standard keys that
// control Apple switches #0/#1.
// - So that in a 2 player game, player 2 can't cheat by controlling player 1's
// buttons.
// If Apple joystick #1 is not NONE, then Apple joystick #0 only gets the use of
// Apple switch #0.
...
// - This is because the 2-joystick version of Mario Bros expects the 2nd
// joystick to control Apple switch #2.
...
static POINT keyvalue[9] = {
    {PDL_MIN, PDL_MAX},     {PDL_CENTRAL, PDL_MAX},     {PDL_MAX, PDL_MAX},
...
DWORD joytype[2] = {DEVICE_JOYSTICK,
                    DEVICE_NONE}; // Emulation Type for joysticks #0 & #1

static BOOL setbutton[3] = {0, 0,
                            0}; // Used when a mouse button is pressed/released

static int xpos[2] = {PDL_CENTRAL, PDL_CENTRAL};
static int ypos[2] = {PDL_CENTRAL, PDL_CENTRAL};

static unsigned __int64 g_nJoyCntrResetCycle =
    0; // Abs cycle that joystick counters were reset
...
void CheckJoyExit() {
  ...
  if (SDL_JoystickGetButton(joy1, joyexitbutton0) &&
      SDL_JoystickGetButton(joy1, joyexitbutton1))
    joyquitevent = true;
  else
    joyquitevent = false;
}
...
void CheckJoystick0() {
  ...
  if ((currtime - lastcheck >= 10) || joybutton[0] || joybutton[1]) {
    ...
    if ((SDL_JoystickGetButton(joy1, joy1button2)) && !joybutton[1] &&
        (joyinfo[joytype[1]].device == DEVICE_NONE) // Only consider 2nd button
                                                    // if NOT emulating a 2nd
                                                    // Apple joystick
        )
      buttonlatch[1] = BUTTONTIME;
    joybutton[0] = SDL_JoystickGetButton(joy1, joy1button1);
    if (joyinfo[joytype[1]].device == DEVICE_NONE) // Only consider 2nd button
                                                   // if NOT emulating a 2nd
                                                   // Apple joystick button
    ...
    // NB. This does not work for analogue joysticks (not self-centreing) -
    // except if Trim=0
  ...
  if ((currtime - lastcheck >= 10) || joybutton[2]) {
    ...
    if (SDL_JoystickGetButton(joy2, joy2button1) && !joybutton[2]) {
      ...
      if (joyinfo[joytype[1]].device != DEVICE_NONE)
        buttonlatch[1] = BUTTONTIME; // Re-map this button when emulating a 2nd
                                     // Apple joystick
    }
    ...
    if (joyinfo[joytype[1]].device != DEVICE_NONE)
      joybutton[1] =
          SDL_JoystickGetButton(joy2, joy2button1); // Re-map this button when
                                                    // emulating a 2nd Apple
                                                    // joystick
...
//===========================================================================
void JoyInitialize() {
// Emulated joystick #0 can only use JOYSTICKID1 (if no joystick, then use
// mouse)
...
#define AXIS_MAX 32767  /* maximum value for axis coordinate */
  ...
  if (joyinfo[joytype[0]].device == DEVICE_JOYSTICK) {
    //    JOYCAPS caps;
    if (number_of_joysticks >
        0) //(joyGetDevCaps(JOYSTICKID1,&caps,sizeof(JOYCAPS)) ==
           //JOYERR_NOERROR)
    {
      joy1 = SDL_JoystickOpen(joy1index); // open joystick and get its
                                          // information into SDL_Joystick
                                          // struct
      joyshrx[0] = 0;
      joyshry[0] = 0;
      joysubx[0] = AXIS_MIN; //(int)caps.wXmin; // just do not know how to get
                             //wXmin and alike from SDL joysticks
  ...
  if (joyinfo[joytype[1]].device == DEVICE_JOYSTICK) {
    //    JOYCAPS caps;
    if (number_of_joysticks >
        1) //(joyGetDevCaps(JOYSTICKID2,&caps,sizeof(JOYCAPS)) ==
           //JOYERR_NOERROR)
    {
      joy2 = SDL_JoystickOpen(joy2index); // open joystick #2 and get its
                                          // information into SDL_Joystick
                                          // struct
      joyshrx[1] = 0;
      joyshry[1] = 0;
...
BOOL JoyProcessKey(int virtkey, BOOL extended, BOOL down, BOOL autorep) {
  BOOL isALT = ((virtkey == SDLK_LALT) |
                (virtkey == SDLK_RALT)); // if either ALT key pressed
  if ((joyinfo[joytype[0]].device != DEVICE_KEYBOARD) &&
      (joyinfo[joytype[1]].device != DEVICE_KEYBOARD) && (!isALT))
    return 0;

  // Joystick # which is being emulated by keyboard
  int nJoyNum = (joyinfo[joytype[0]].device == DEVICE_KEYBOARD) ? 0 : 1;
  int nCenteringType =
      joyinfo[joytype[nJoyNum]].mode; // MODE_STANDARD or MODE_CENTERING
  ...
  if (isALT /*virtkey == VK_MENU*/) // VK_MENU == ALT Key (Button #0 or #1)
  {
    ...
    if ((virtkey >= SDLK_KP1) && (virtkey <= SDLK_KP9)) {
      ...
      switch (virtkey) { // Here VK_PRIOR means “Page Up”, VK_NEXT is “Page
                         // Down”
      case SDLK_END:
        keydown[0] = down;
        break;
      ...
      case SDLK_CLEAR:
        keydown[4] = down;
        break; // where is that clear key???? I don't have one! --bb
      case SDLK_RIGHT:
        keydown[5] = down;
        break;
      case SDLK_HOME:
    ...
    if ((virtkey == SDLK_KP0) || (virtkey == SDLK_INSERT)) {
      if (down) {
        if (joyinfo[joytype[1]].device != DEVICE_KEYBOARD) {
          ...
          buttonlatch[2] = BUTTONTIME;
          buttonlatch[1] = BUTTONTIME; // Re-map this button when emulating a
                                       // 2nd Apple joystick
        }
      }
...
BYTE /*__stdcall */ JoyReadButton(WORD, WORD address, BYTE, BYTE,
                                  ULONG nCyclesLeft) {
  address &= 0xFF;
  ...
  case 0x61:
    pressed = (buttonlatch[0] || joybutton[0] || setbutton[0] ||
  ...
  case 0x62:
    pressed = (buttonlatch[1] || joybutton[1] || setbutton[1] ||
...
//  EA       : (2) NOP
//  EA       : (2) NOP
// Lbl1:						; 11 cycles is the normal duration of the
// loop
//  BD 64 C0 : (4) LDA $C064,X
//  10 04    : (2) BPL Lbl2		; NB. 3 cycles if branch taken (not
//  likely)
//  C8       : (2) INY
//  D0 F8    : (3) BNE Lbl1		; NB. 2 cycles if branck not taken (not
//  likely)
//  88       : (2) DEY
// Lbl2:
//  60       : (6) RTS
...
BYTE /*__stdcall*/ JoyReadPosition(WORD programcounter, WORD address, BYTE,
                                   BYTE, ULONG nCyclesLeft) {
  ...
  // This is from KEGS. It helps games like Championship Lode Runner &
  // Boulderdash
  if (nPdlPos >= 255)
    nPdlPos = 280;
  ...
  BOOL nPdlCntrActive =
      ...
      (g_nJoyCntrResetCycle +
       (unsigned __int64)((double)nPdlPos * PDL_CNTR_INTERVAL));
...
BOOL JoySetEmulationType(/*HWND window,*/ DWORD newtype, int nJoystickNumber) {
  ...
  if (joyinfo[newtype].device == DEVICE_JOYSTICK) {
    //    JOYCAPS caps;
    //	unsigned int nJoyID = nJoystickNumber == JN_JOYSTICK0 ? JOYSTICKID1 :
    //JOYSTICKID2;
    ...
    if (sg_Mouse.Active()) {
      /*MessageBox(window,
                             TEXT("Mouse interface card is enabled - unable to
         use mouse for joystick emulation."),
                             TEXT("Configuration"),
                             MB_ICONEXCLAMATION | MB_SETFOREGROUND);*/
      fprintf(stderr, "Mouse interface card is enabled - unable to use mouse "
...
//===========================================================================
BOOL JoyUsingMouse() {
  return (joyinfo[joytype[0]].device == DEVICE_MOUSE) ||
         (joyinfo[joytype[1]].device == DEVICE_MOUSE);
}
...
short JoyGetTrim(bool bAxisX) { return bAxisX ? g_nPdlTrimX : g_nPdlTrimY; }
