...
void KeybQueueKeypress(int key, BOOL bASCII) {
  ...
  if (bASCII == ASCII) {
    /*		if (bFreshReset && key == 0x03)
                    {
                            bFreshReset = 0;
                            return; // Swallow spurious CTRL-C caused by
       CTRL-BREAK
                    }
                    bFreshReset = 0;*/
    if (key > 0x7F)
    ...
    in Frame.cpp
                    {
                            ...
                            KeybReset();
                            if (!IS_APPLE2)
                                    VideoResetState();	// Switch Alternate
    char set off
                            MB_Reset();
    ...
    */
    ...
    // 		if (!((key >= VK_LEFT) && (key <= VK_DELETE) && asciicode[IS_APPLE2 ? 0
    // : 1][key - VK_LEFT]))
    // 			return;
    // 		keycode = asciicode[IS_APPLE2 ? 0 : 1][key - VK_LEFT];		// Convert to
    // Apple arrow keycode
    // 		lastvirtkey = key;
