...
/* reference: technote tn-iigs-063 "Master Color Values"
          ...
          Name       Value      #  #   #      Value
          ----------------------------------------------------
          Black       0         0  0,4 0      $0000    (0,0,0) -> (00,00,00)
Windows
(Magenta) Deep Red    1         1      1      $0D03    (D,0,3) -> (D0,00,30)
Custom
          Dark Blue   2         2      8      $0009    (0,0,9) -> (00,00,80)
Windows
 (Violet) Purple      3         3  2   9      $0D2D    (D,2,D) -> (FF,00,FF)
Windows
          Dark Green  4         4      4      $0072    (0,7,2) -> (00,80,00)
Windows
 (Gray 1) Dark Gray   5         5      5      $0555    (5,5,5) -> (80,80,80)
Windows
   (Blue) Medium Blue 6         6  6   C      $022F    (2,2,F) -> (00,00,FF)
Windows
   (Cyan) Light Blue  7         7      D      $06AF    (6,A,F) -> (60,A0,FF)
Custom
          Brown       8         8      2      $0850    (8,5,0) -> (80,50,00)
Custom
          Orange      9         9  5   3      $0F60    (F,6,0) -> (FF,80,00)
Custom (modified to match better with the other Hi-Res Colors)
 (Gray 2) Light Gray  A         A      A      $0AAA    (A,A,A) -> (C0,C0,C0)
Windows
          Pink        B         B      B      $0F98    (F,9,8) -> (FF,90,80)
Custom
  (Green) Light Green C         C  1   6      $01D0    (1,D,0) -> (00,FF,00)
Windows
          Yellow      D         D      7      $0FF0    (F,F,0) -> (FF,FF,00)
Windows
   (Aqua) Aquamarine  E         E      E      $04F9    (4,F,9) -> (40,FF,90)
Custom
          White       F         F  3,7 F      $0FFF    (F,F,F) -> (FF,FF,FF)
Windows
...
void DrawMonoTextSource(SDL_Surface *dc); // yes, we have just SDL_Surface
                                          // either for DeviceContext, or bitmap
void DrawTextSource(SDL_Surface *dc);

//===========================================================================
void /*__stdcall */ CopySource(int destx, int desty, int xsize, int ysize,
  ...
  while (ysize--) {
    ...
    while (bytesleft) {
      bytesleft -= 4;
      *(LPDWORD)(currdestptr + bytesleft) =
          *(LPDWORD)(currsourceptr + bytesleft);
    }
    currdestptr += framebufferpitch; // we are going top to bottom, as all
                                     // normal people do! ^_^ (bb)
...
void CreateFrameOffsetTable(LPBYTE addr, LONG /*int*/ pitch) {
  // as I could take it's just needed for windzooeee DD while in FullScreen
  // mode.
  ...
  for (int loop = 0; loop < 384; loop++)
    frameoffsettable[loop] =
        framebufferaddr + framebufferpitch * loop; //(383-loop);
}
...
void CreateDIBSections() {
  ...
  g_hDeviceBitmap =
      SDL_CreateRGBSurface(SDL_SWSURFACE, 560, 384, 8, 0, 0, 0, 0);

  g_origscreen = SDL_CreateRGBSurface(SDL_SWSURFACE, g_ScreenWidth,
                                      g_ScreenHeight, 8, 0, 0, 0, 0);
  ...
  g_hStatusSurface = SDL_CreateRGBSurface(
      SDL_SWSURFACE, STATUS_PANEL_W, STATUS_PANEL_H, SCREEN_BPP, 0, 0, 0, 0);
  ...
  g_hSourceBitmap = SDL_CreateRGBSurface(SDL_SWSURFACE, SRCOFFS_TOTAL,
                                         MAX_SOURCE_Y, 8, 0, 0, 0, 0);
  ...
  for (int y = 0; y < MAX_SOURCE_Y; y++)
    g_aSourceStartofLine[y] =
        g_pSourcePixels + SRCOFFS_TOTAL * y; //((MAX_SOURCE_Y-1) - y);
  ...
  ZeroMemory(
      g_pSourcePixels,
      SRCOFFS_TOTAL *
          /*512*/ MAX_SOURCE_Y); // be consistent, please,Thom! (bb) ^_^ ku
...
const BYTE aColorIndex[NUM_COLOR_MAPPING] = {HGR_MAGENTA, HGR_BLUE,  HGR_GREEN,
                                             HGR_RED,     HGR_BLACK, HGR_WHITE};
      ...
      while (x < 28) {
        ...
        for (iPixel = 2; iPixel < 9; iPixel++) {
          ...
          /*
                  ...
                  @reference: see Beagle Bro's Disk: "Silicon Salid", File:
             DOUBLE HI-RES
                  Fortunately double-hires is supported via pixel doubling, so
             we can do half-pixel shifts ;-)
          */
          switch (color) {
          case CM_Magenta:
            SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj, y,
                           HGR_MAGENTA); // aColorIndex
            SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj + 1, y,
                           DARK_MAGENTA); // aColorDimmedIndex
          ...
          case CM_Orange:
            ...
            SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj + 2, y,
                           BROWN); // DARK_RED is a bit "too" red
            SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj + 1, y + 1,
                           HGR_RED);
            SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj + 2, y + 1,
                           BROWN); // DARK_RED is a bit "too" red
          ...
          case CM_White:
...
#if HALF_SHIFT_DIM
            // 50% dither -- would look OK, except Gumball, on the "Gumball"
            // font has splotches
            //							SETSOURCEPIXEL(SRCOFFS_HIRES+coloffs+x+adj+1,y  ,
            //HGR_WHITE );
            //							SETSOURCEPIXEL(SRCOFFS_HIRES+coloffs+x+adj+1,y+1,
            ...
            // 75% dither -- looks kind of nice actually.  Passes the Gumball
            // cutscene quality test!
            ...
            SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj + 1, y, HGR_WHITE);
            SETSOURCEPIXEL(
                SRCOFFS_HIRES + coloffs + x + adj, y + 1,
                HGR_WHITE); // LIGHT_GRAY <- for that half scan-line look
            SETSOURCEPIXEL(
                SRCOFFS_HIRES + coloffs + x + adj + 1, y + 1,
                HGR_WHITE); // LIGHT_GRAY <- for that half scan-line look
            // Prevent column gaps
            if (hibit) {
              if (iPixel <= 2) {
                SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj, y,
                               HGR_WHITE); // LIGHT_GRAY HGR_GREY1
                SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj, y + 1,
                               HGR_WHITE); // LIGHT_GRAY HGR_GREY1
              }
            }
        ...
        for (iPixel = 2; iPixel < 9; iPixel++) {
          ...
          if (aPixels[iPixel]) {
            ...
            ***/
            ...
            if ((videotype == VT_COLOR_STANDARD) ||
                (videotype == VT_COLOR_TVEMU) ||
                !(aPixels[iPixel - 2] && aPixels[iPixel + 2]))
              color = ((odd ^ !(iPixel & 1)) << 1) |
                      hibit; // // No white HGR text optimization
          }
          ...
          // cBL cBR
          SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj, y,
                         aColorIndex[color]); // TL
          SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj + 1, y,
                         aColorIndex[color]); // TR
          SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj, y + 1,
                         aColorIndex[color]); // BL
          SETSOURCEPIXEL(SRCOFFS_HIRES + coloffs + x + adj + 1, y + 1,
                         aColorIndex[color]); // BR
          x += 2;
        }
      }
    }
...
void SetLastDrawnImage() {
  ...
  else if (SW_80COL) // Don't test for !SW_HIRES, as some 80-col text routines
                     // have SW_HIRES set (Bug #8300)
...
bool Update40ColCell(int x, int y, int xpixel, int ypixel, int offset) {
  ...
  if (bCharChanged || (bCharFlashing && g_bTextFlashFlag)) {
    ...
    CopySource(xpixel, ypixel, APPLE_FONT_WIDTH, APPLE_FONT_HEIGHT,
               (IS_APPLE2 ? SRCOFFS_IIPLUS : SRCOFFS_40COL) +
                   ((ch & 0x0F) << 4),
...
inline bool _Update80ColumnCell(BYTE c, const int xPixel, const int yPixel,
                                bool bCharFlashing) {
  bool bInvert = bCharFlashing ? g_bTextFlashState : false;

  CopySource(xPixel, yPixel, (APPLE_FONT_WIDTH / 2), APPLE_FONT_HEIGHT,
...
bool Update80ColCell(int x, int y, int xpixel, int ypixel, int offset) {
  ...
  if ((auxval != *(vidlastmem + offset)) ||
      (mainval != *(vidlastmem + offset + 0x400)) || redrawfull) {
    CopySource(xpixel, ypixel, (APPLE_FONT_WIDTH / 2), APPLE_FONT_HEIGHT,
               SRCOFFS_80COL + ((auxval & 15) << 3),
               ((auxval >> 4) << 4) + g_nAltCharSetOffset);

    CopySource(xpixel + 7, ypixel, (APPLE_FONT_WIDTH / 2), APPLE_FONT_HEIGHT,
...
bool UpdateDHiResCell(int x, int y, int xpixel, int ypixel, int offset) {
  ...
  while (yoffset < 0x2000) {
    ...
    if ((byteval2 != *(vidlastmem + offset + yoffset)) ||
        ...
        ((x > 0) && ((byteval1 & 0x70) !=
                     (*(vidlastmem + offset + yoffset + 0x1FFF) & 0x70))) ||
        ((x < 39) && ((byteval4 & 0x07) !=
                      (*(vidlastmem + offset + yoffset + 1) & 0x07))) ||
        redrawfull) {
...
#define VALUE (dwordval >> (4 + PIXEL - COLOR))
      CopySource(xpixel + PIXEL, ypixel + (yoffset >> 9), 7, 2,
                 SRCOFFS_DHIRES + 10 * HIBYTE(VALUE) + COLOR,
                 LOBYTE(VALUE) << 1);
#undef PIXEL
#define PIXEL 7
      CopySource(xpixel + PIXEL, ypixel + (yoffset >> 9), 7, 2,
...
//===========================================================================
void /*__stdcall */ MixColorsVertical(int matx,
                                      int maty) { // For tv emulation g_nAppMode
...
//===========================================================================
void /*__stdcall */ CopyMixedSource(
    int x, int y, int sourcex, int sourcey) { // For tv emulation g_nAppMode
  ...
  // transfer 7 pixels (i.e. the visible part of an apple hgr-byte) from row to
  // pixelmatrix
  for (count = 0, bufxoffset = 0; count < 7; count++, bufxoffset += 2) {
    ...
    if (hgrlinesabove)
      currptr -= framebufferpitch
                 << 1; // we just should change sign of op: '-' instead of '+'
...
bool UpdateHiResCell(int x, int y, int xpixel, int ypixel, int offset) {
  ...
  while (yoffset < 0x2000) {
    ...
    if ((byteval2 != *(vidlastmem + offset + yoffset + 0x2000)) ||
        ((x > 0) && ((byteval1 & 0x60) !=
                     (*(vidlastmem + offset + yoffset + 0x1FFF) & 0x60))) ||
        ((x < 39) && ((byteval3 & 0x03) !=
                      (*(vidlastmem + offset + yoffset + 0x2001) & 0x03))) ||
        redrawfull) {
...
void VideoBenchmark() {
  ...
  {
    printf("The emulator has detected a problem while running the CPU "
           "benchmark.\n");
    ...
    if (error) {
      ...
      printf("The emulator experienced an error %u clock cycles into the CPU "
             "benchmark.\n",
             (unsigned)loop);
      ...
      printf("This probably means that the problem is external to the "
             "emulator, happening asynchronously,\n");
...
//===========================================================================
BYTE /*__stdcall*/ VideoCheckMode(WORD, WORD address, BYTE, BYTE,
                                  ULONG nCyclesLeft) {
  address &= 0xFF;
  if (address == 0x7F)
...
void VideoChooseColor() { // will implement later. May be...???? ^_^
  ...
  //       VideoRedrawScreen();
  //     RegSaveValue(TEXT("Configuration"),TEXT("Monochrome
  //     Color"),1,monochrome);
  //   }
}
...
void VideoDisplayLogo() {
  ...
  if (screen->format->palette && g_hLogoBitmap->format->palette)
    SDL_SetColors(screen, g_hLogoBitmap->format->palette->colors, 0,
                  g_hLogoBitmap->format->palette->ncolors);
  ...
  // 	char* szVersion = new char[strlen(VERSION_TXT) + strlen(VERSIONSTRING) +
  // 1];
  ...
  // 										strlen(szVersion));
  //
  // 	if (GetDeviceCaps(hFrameDC,PLANES) * GetDeviceCaps(hFrameDC,BITSPIXEL)
  // <= 4) {
...
void VideoRefreshScreen() {
  ...
  LONG /*int*/ pitch = 560; // pitch stands for pixels in a row, if one pixel
                            // stands for one byte (560 in our case)
  // I could take pitch such: LONG pitch = screen->pitch; . May be it would be
  // better, what'd you think? --bb
  ...
  UpdateFunc_t update =
      SW_TEXT
          ? SW_80COL ? Update80ColCell : Update40ColCell
          : SW_HIRES
                ? (SW_DHIRES && SW_80COL) ? UpdateDHiResCell : UpdateHiResCell
                : (SW_DHIRES && SW_80COL) ? UpdateDLoResCell : UpdateLoResCell;
  ...
  if (g_iStatusCycle > 0) {
    //		double led_fades[SHOW_CYCLES + 1] =
    //{0.20,0.30,0.40,0.50,0.60,0.70,0.80,0.85,0.88,0.92,0.95,0.94,0.96,0.97,0.98,0.99};
    //		double nowleds = led_fades[g_iStatusCycle];
    //		printf("g_iStatusCycle=%d\tLeds: %f\n",g_iStatusCycle,nowleds);
    //		surface_fader(g_hStatusSurface, nowleds, nowleds, nowleds, -1,
    //0);
...
//===========================================================================
BYTE /*__stdcall*/ VideoSetMode(WORD, WORD address, BYTE write, BYTE,
                                ULONG nCyclesLeft) {
  address &= 0xFF;
  ...
  if (SW_MASK2)
    vidmode &= ~VF_PAGE2;
  if (oldvalue !=
      g_nAltCharSetOffset + (int)(vidmode & ~(VF_MASK2 | VF_PAGE2))) {
...
void VideoUpdateFlash() {
  ...
  if (nTextFlashCnt == 60 / 6) // Flash rate = 6Hz (every 166ms)
  {
    ...
    // Redraw any FLASHing chars if any text showing. NB. No FLASH g_nAppMode
    // for 80 cols
...
bool VideoGetSW80COL() { return SW_80COL ? true : false; }
...
WORD VideoGetScannerAddress(bool *pbVblBar_OUT, const DWORD uExecutedCycles) {
  ...
  //
  if (SW_HIRES && SW_MIXED && (v_4 & v_2)) // NICK: Should this be (SW_HIRES &&
                                           // !SW_TEXT) instead of just
                                           // 'SW_HIRES' ?
  {
