...
bool DSInit() {
  ...
  g_bDSAvailable = SDLSoundDriverInit(
      SPKR_SAMPLE_RATE, 4096); // I just do not know what number of samples use.
...
// be courteous with BIG_Endian systems, please! --bb
  ...
  /*
          if (SDL_InitSubSystem(SDL_INIT_AUDIO) != 0) {
                  printf("Unable to initialize SDL audio subsystem: %s",
     SDL_GetError());
                  return false;
          }
  */
  ...
  //	bufferSize = SPKR_SAMPLE_RATE * 2 * sizeof(short);	// 1 second of
  //stereo short data
