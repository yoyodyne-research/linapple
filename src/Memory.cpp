...
bool MemCheckSLOTCXROM() { return SW_SLOTCXROM != 0; }
...
LPBYTE MemGetAuxPtr(WORD offset) {
  ...
  if (((SW_PAGE2 && SW_80STORE) || VideoGetSW80COL()) &&
      ((((offset & 0xFF00) >= 0x0400) && ((offset & 0xFF00) <= 0x700)) ||
...
#define IDR_APPLE2E_ENHANCED_ROM "Apple2e_Enhanced.rom"
  ...
  if (RomFileName == NULL) {
    /*    TCHAR sRomFileName[ MAX_PATH ];
        switch (g_Apple2Type)
        {
        case A2TYPE_APPLE2:      _tcscpy(sRomFileName, TEXT("APPLE2.ROM"));
       break;
        case A2TYPE_APPLE2PLUS:    _tcscpy(sRomFileName,
       TEXT("APPLE2_PLUS.ROM")); break;
        case A2TYPE_APPLE2E:    _tcscpy(sRomFileName, TEXT("APPLE2E.ROM"));
       break;
       ...
       TEXT("APPLE2E_ENHANCED.ROM")); break;
        ...
        wsprintf( sText, TEXT("Unable to open the required firmware ROM data
       file.\n\nFile: %s"), sRomFileName );
