...
// - Changed MB output so L=All voices from AY0 & AY2 & R=All voices from AY1 &
// AY3
...
static USHORT g_nMBTimerDevice =
    0; // SY6522 device# which is generating timer IRQ
static unsigned __int64 g_uLastCumulativeCycles = 0;

#ifdef MB_SPEECH
// SSI263 vars:
static USHORT g_nSSI263Device =
    0; // SSI263 device# which is generating phoneme-complete IRQ
...
// static const DWORD SAMPLE_RATE = 44100;  // Use a base freq so that DirectX
// (or sound h/w) doesn't have to up/down-sample
...
static const DWORD g_dwDSBufferSize =
    16 * 1024 * sizeof(short) * g_nMB_NumChannels;
...
#ifdef MB_SPEECH
static HANDLE g_hSSI263Event[g_nNumEvents] = {
    NULL}; // 1: Phoneme finished playing, 2: Exit thread
...
static const double g_f6522TimerPeriod_NoIRQ =
    CLK_6502 / 60.0; // Constant whatever the CLK is set to
...
static void UpdateIFR(SY6522_AY8910 *pMB) {
  ...
  // . SSI263's IRQ (A/!R) is routed via the 2nd 6522 (at $Cx80) and must
  // generate a 6502 IRQ (not NMI)
  // . SC-01's IRQ (A/!R) is also routed via a (2nd?) 6522
  // Phasor's SSI263 appears to be wired directly to the 6502's IRQ (ie. not via
  // a 6522)
...
#if LOG_SSI263
    if (g_fh)
      fprintf(g_fh, "DUR   = 0x%02X, PHON = 0x%02X\n\n", nValue >> 6,
              nValue & PHONEME_MASK);
#endif
...
#if LOG_SSI263
    if (g_fh)
      fprintf(g_fh, "RATE  = 0x%02X, INF = 0x%02X\n", nValue >> 4,
              nValue & 0x0F);
#endif
...
#if LOG_SSI263
    if (g_fh)
      fprintf(g_fh, "CTRL  = %d, ART = 0x%02X, AMP=0x%02X\n", nValue >> 7,
              (nValue & ARTICULATION_MASK) >> 4, nValue & AMPLITUDE_MASK);
#endif
    if ((pMB->SpeechChip.CtrlArtAmp & CONTROL_MASK) &&
        !(nValue & CONTROL_MASK)) // H->L
      pMB->SpeechChip.CurrentMode =
          pMB->SpeechChip.DurationPhonome & DURATION_MODE_MASK;
    pMB->SpeechChip.CtrlArtAmp = nValue;
    break;
  case SSI_FILFREQ:
#if LOG_SSI263
    if (g_fh)
      fprintf(g_fh, "FFREQ = 0x%02X\n", nValue);
#endif
  ...
  if (!g_bMB_RegAccessedFlag) {
    if (!g_nMB_InActiveCycleCount) {
      g_nMB_InActiveCycleCount = g_nCumulativeCycles;
               ...
               (unsigned __int64)g_fCurrentCLK6502 / 10) {
...
//
  ...
  int nNumSamplesPerPeriod =
      (int)((double)SAMPLE_RATE / nIrqFreq); // Eg. For 60Hz this is 735
  ...
  if (nNumSamples)
    for (int nChip = 0; nChip < NUM_AY8910; nChip++)
      AY8910Update(nChip, &ppAYVoiceBuffer[nChip * NUM_VOICES_PER_AY8910],
                   nNumSamples);
  ...
  for (int i = 0; i < nNumSamples; i++) {
    ...
    for (unsigned int j = 0; j < NUM_VOICES_PER_AY8910; j++) {
      // Slot4
      nDataL +=
          (int)((double)ppAYVoiceBuffer[0 * NUM_VOICES_PER_AY8910 + j][i] *
                fAttenuation);
      nDataR +=
          (int)((double)ppAYVoiceBuffer[1 * NUM_VOICES_PER_AY8910 + j][i] *
                fAttenuation);

      // Slot5
      nDataL +=
          (int)((double)ppAYVoiceBuffer[2 * NUM_VOICES_PER_AY8910 + j][i] *
                fAttenuation);
      nDataR +=
          (int)((double)ppAYVoiceBuffer[3 * NUM_VOICES_PER_AY8910 + j][i] *
                fAttenuation);
    }
  ...
  // now we have sample data in g_nMixBuffer of size nNumSamples?? Ok, upload it
  // for playing
  // NOTE: when you delete the comment of the line below, speakers will work
  // badly, but Mockingboard should work?
...
static DWORD SSI263Thread(LPVOID lpParameter) {
  while (1) {
    DWORD dwWaitResult =
        WaitForMultipleObjects(g_nNumEvents,   // number of handles in array
                               g_hSSI263Event, // array of event handles
                               FALSE,          // wait until any one is signaled
                               INFINITE);

    if ((dwWaitResult < WAIT_OBJECT_0) ||
        (dwWaitResult > WAIT_OBJECT_0 + g_nNumEvents - 1))
      continue;
...
// g_nCurrentActivePhoneme);
    ...
    if (g_bPhasorEnable) {
      if ((pMB->SpeechChip.CurrentMode != MODE_IRQ_DISABLED)) {
        ...
        // Phasor's SSI263.IRQ line appears to be wired directly to IRQ
        // (Bypassing the 6522)
...
//
  ...
  if ((g_hSSI263Event[0] == NULL) || (g_hSSI263Event[1] == NULL)) {
    if (g_fh)
      fprintf(g_fh, "SSI263: CreateEvent failed\n");
    return false;
  }
  ...
  for (int i = 0; i < 64; i++) {
    ...
    unsigned int nPhonemeByteLength =
        g_nPhonemeInfo[nPhoneme].nLength * sizeof(SHORT);

    // NB. DSBCAPS_LOCSOFTWARE required for Phoneme+2==0x28 - sample too short
    // (see KB327698)
    hr = DSGetSoundBuffer(&SSI263Voice[i],
                          DSBCAPS_CTRLVOLUME + DSBCAPS_CTRLPOSITIONNOTIFY +
                              DSBCAPS_LOCSOFTWARE,
                          nPhonemeByteLength, 22050, 1);
    if (FAILED(hr)) {
      if (g_fh)
        fprintf(g_fh, "SSI263: DSGetSoundBuffer failed (%08X)\n", hr);
      return false;
    }

    hr = DSGetLock(SSI263Voice[i].lpDSBvoice, 0, 0, &pDSLockedBuffer,
                   &dwDSLockedBufferSize, NULL, 0);
    if (FAILED(hr)) {
      if (g_fh)
        fprintf(g_fh, "SSI263: DSGetLock failed (%08X)\n", hr);
      return false;
    }

    if (bPause) {
      // 'pause' length is length of 1st phoneme (arbitrary choice, since don't
      // know real length)
      memset(pDSLockedBuffer, 0x00, nPhonemeByteLength);
    } else {
      memcpy(pDSLockedBuffer, &g_nPhonemeData[g_nPhonemeInfo[nPhoneme].nOffset],
             nPhonemeByteLength);
    }

    hr = SSI263Voice[i].lpDSBvoice->QueryInterface(
        IID_IDirectSoundNotify, (LPVOID *)&SSI263Voice[i].lpDSNotify);
    if (FAILED(hr)) {
      if (g_fh)
        fprintf(g_fh, "SSI263: QueryInterface failed (%08X)\n", hr);
      return false;
    }
    ...
    hr =
        SSI263Voice[i].lpDSNotify->SetNotificationPositions(1, &PositionNotify);
    if (FAILED(hr)) {
      if (g_fh)
        fprintf(g_fh, "SSI263: SetNotifyPos failed (%08X)\n", hr);
      return false;
    }

    hr = SSI263Voice[i].lpDSBvoice->Unlock((void *)pDSLockedBuffer,
                                           dwDSLockedBufferSize, NULL, 0);
    if (FAILED(hr)) {
      if (g_fh)
        fprintf(g_fh, "SSI263: DSUnlock failed (%08X)\n", hr);
      return false;
    }
...
static BYTE /*__stdcall*/ PhasorIO(WORD PC, WORD nAddr, BYTE bWrite,
                                   BYTE nValue, ULONG nCyclesLeft);
static BYTE /*__stdcall*/ MB_Read(WORD PC, WORD nAddr, BYTE bWrite, BYTE nValue,
                                  ULONG nCyclesLeft);
static BYTE /*__stdcall*/ MB_Write(WORD PC, WORD nAddr, BYTE bWrite,
                                   BYTE nValue, ULONG nCyclesLeft);
...
void MB_Initialize() {
  if (g_bDisableDirectSound) {
    ...
    for (i = 0; i < NUM_VOICES; i++)
      ppAYVoiceBuffer[i] = new short[SAMPLE_RATE]; // Buffer can hold a max of 1
...
void MB_Reinitialize() { AY8910_InitClock((int)g_fCurrentCLK6502); }
...
static BYTE /*__stdcall*/ MB_Write(WORD PC, WORD nAddr, BYTE bWrite,
  ...
  if (g_bPhasorEnable) {
    ...
    if (CS & 1)
      SY6522_Write(nMB * NUM_DEVS_PER_MB + SY6522_DEVICE_A, nAddr & 0xf,
                   nValue);

    if (CS & 2)
      SY6522_Write(nMB * NUM_DEVS_PER_MB + SY6522_DEVICE_B, nAddr & 0xf,
                   nValue);

    if ((nOffset >= SSI263_Offset) && (nOffset <= (SSI263_Offset + 0x05)))
      SSI263_Write(nMB * 2 + 1, nAddr & 0xf,
                   nValue); // Second 6522 is used for speech chip
...
void MB_UpdateCycles(ULONG uExecutedCycles) {
  ...
  for (int i = 0; i < NUM_SY6522; i++) {
    ...
    // bool bTimer2Underflow = (!(OldTimer2 & 0x8000) &&
    // (pMB->sy6522.TIMER2_COUNTER.w & 0x8000));
...
eSOUNDCARDTYPE MB_GetSoundcardType() { return g_SoundcardType; }
...
//
    ...
    if (pMB->SpeechChip.DurationPhonome) {
      ...
      if ((pMB->SpeechChip.CurrentMode != MODE_IRQ_DISABLED) &&
          (pMB->sy6522.PCR == 0x0C) && (pMB->sy6522.IER & IxR_PERIPHERAL)) {
        pMB->sy6522.IFR |= IxR_PERIPHERAL;
        UpdateIFR(pMB);
        pMB->SpeechChip.CurrentMode |= 1; // Set SSI263's D7 pin
      }
