...
// 	  g_uScrollLockToggle = dwTmp;
  ...
  if(RegLoadString(TEXT("Configuration"), TEXT("Monochrome Color"), 1, &sz, 10))
  {
	  if (!sscanf(sz, "#%X", &monochrome)) monochrome = 0xC0C0C0;
	  free(sz);
	  sz = NULL;
  }
  ...
  if (RegLoadString(TEXT("Configuration"),TEXT("Screen factor"),1, &sz,16)) {
  ...
  if (RegLoadString(TEXT("Configuration"),TEXT(REGVALUE_SAVESTATE_FILENAME),1, &sz,MAX_PATH)) {
  	Snapshot_SetFilename(sz);
	free(sz);
	sz = NULL;
  }

  RegLoadString(TEXT("Preferences"), REGVALUE_PREF_START_DIR, 1, &sz, MAX_PATH);
  if (sz) {
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_PREF_HDD_START_DIR, 1, &sz, MAX_PATH);
  if (sz) {
	  strcpy(g_sHDDDir, sz);
	  free_null(sz);
  }
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_PREF_SAVESTATE_DIR, 1, &sz, MAX_PATH);
  if (sz) {
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_FTP_HDD_DIR, 1, &sz, MAX_PATH);
  if (sz) {
  ...
  RegLoadString(TEXT("Preferences"), REGVALUE_FTP_LOCAL_DIR, 1, &sz, MAX_PATH);
  if (sz) {
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		if(cli.benchmark) VideoBenchmark(); // start VideoBenchmark and exit
