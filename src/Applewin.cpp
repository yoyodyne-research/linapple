...
void SetDiskImageDirectory(char *regKey, int driveNumber) {
  ...
  if (RegLoadString(TEXT("Configuration"), TEXT(regKey), 1, &szHDFilename,
                    MAX_PATH)) {
    if (!ValidateDirectory(szHDFilename)) {
      RegSaveString(TEXT("Configuration"), TEXT(regKey), 1, "/");
      RegLoadString(TEXT("Configuration"), TEXT(regKey), 1, &szHDFilename,
                    MAX_PATH);
    }
...
// Sets the emulator to automatically boot, rather than load the flash screen on
// startup
void setAutoBoot() {
  // autostart
  SDL_Event user_ev;
...
void LoadConfiguration() {
  ...
  if (registry == NULL) {
    printf("File " REGISTRY
           " could not be opened. Using default configuration.\n");
    return;
  }
...
void RegisterExtensions() { // TO DO: register extensions for KDE or GNOME
  ...
  //   wsprintf(command, "\"%s\"",  szCommandTmp);  // Wrap  path & filename  in
  //   quotes &  null terminate
  //
  //   TCHAR icon[MAX_PATH];
...
//     }
  ...
  do {
    ...
    // Start with default configuration, which we will override if command line
    // options were specified
    LoadConfiguration();

    // Overwrite configuration file's set fullscreen option, if one was
    // specified on the command line
    if (bSetFullScreen) {
      fullscreen = bSetFullScreen;
    }

    // This part of the code inserts disks if any were specified on the command
    // line, overwriting the
