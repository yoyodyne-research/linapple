...
DWORD CSuperSerialCard::CommThread(LPVOID lpParameter) {
  ...
  #if 1
    HANDLE hCommEvent_Wait[nNumEvents] = {pSSC->m_hCommEvent[COMMEVT_WAIT],
  ...
  pSSC->m_hCommEvent[COMMEVT_TERM]};
    ...
    while(1)
    {
      ...
      bRes = WaitCommEvent(pSSC->m_hCommHandle, &dwEvtMask, &pSSC->m_o);  //
  Will return immediately (probably with ERROR_IO_PENDING)
      _ASSERT(!bRes);
      if (!bRes)
      {
        ...
        while(1)
        {
          ...
          // On very 1st wait *only*: get a false signal (when not running via
  debugger)
          if ((dwWaitResult == WAIT_OBJECT_0) && (dwEvtMask == 0))
            continue;

          if ((dwWaitResult >= WAIT_OBJECT_0) && (dwWaitResult <=
  WAIT_OBJECT_0+nNumEvents-1))
            break;
        }
...
void CSuperSerialCard::CommThUninit() {
  /*  if (m_hCommThread)
    {
      SetEvent(m_hCommEvent[COMMEVT_TERM]);  // Signal to thread that it should
    exit
