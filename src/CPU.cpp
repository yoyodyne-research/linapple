...
// .         STZ probably doesn't add a cycle since otherwise it would be slower
// than STA which doesn't make sense.
//
...
//
// 65C02 info:
// .  Read-modify-write instructions abs indexed in same page take 6 cycles (cf.
// 7 cycles for 6502)
// .  ASL, DEC, INC, LSR, ROL, ROR
// .  This should work now (but makes bSlowerOnPagecross even less useful for
// r/w detection)
//
// . Thanks to Scott Hemphill for the verified CMOS ADC and SBC algorithm! You
// rock.
// . And thanks to the VICE team for the NMOS ADC and SBC algorithms as well as
// the
...
// TODO: Use IRQ_CHECK_TIMEOUT=128 when running at full-speed else with
// IRQ_CHECK_TIMEOUT=1
// - What about when running benchmark?
// GPH Dropped to IRQ_CHECK_TIMOUT=16 - Mockingboard-intensive applications
// sound
  ...
  {                                                                            \
    ...
    if (page)                                                                  \
      *(page + (addr & 0xFF)) = (BYTE)(a);                                     \
      ...
      IOWrite[(addr >> 4) & 0xFF](regs.pc, addr, 1, (BYTE)(a),                 \
                                  uExecutedCycles);                            \
  }
...
#define ABS                                                                    \
  addr = *(LPWORD)(mem + regs.pc);                                             \
  regs.pc += 2;
#define IABSX                                                                  \
  addr = *(LPWORD)(mem + (*(LPWORD)(mem + regs.pc)) + (WORD)regs.x);           \
  regs.pc += 2;
#define ABSX                                                                   \
...
#define ASO                                                                    \
  ...
  SETNZ(regs.a)
#define AXA                                                                    \
  bSlowerOnPagecross = 0; /*FIXME: $93 case is still unclear*/                 \
...
#define ROR_CMOS                                                               \
  ...
  WRITE(val)
#define RORA                                                                   \
  val = (((WORD)regs.a) >> 1) | (flagc ? 0x80 : 0);                            \
  flagc = (regs.a & 1);                                                        \
  regs.a = val & 0xFF;                                                         \
  SETNZ(regs.a)
#define RRA                                                                    \
...
void RequestDebugger() {
  ...
  // out of sync
  // due to EnterMessageLoop() calling ContinueExecution() after the mode has
  // changed to DEBUG.
  //  DebugBegin();
...
bool CheckDebugBreak(int iOpcode) {
  ...
  if ((g_nAppMode != MODE_DEBUG) && (g_nAppMode != MODE_STEPPING)) {
    if (((iOpcode == 0) && IsDebugBreakOnInvalid(0)) ||
        ((g_iDebugOnOpcode) &&
         (g_iDebugOnOpcode ==
          iOpcode))) // User wants to enter debugger on opcode?
    {
      RequestDebugger();
      return true;
    }
...
#define INV                                                                    \
  if (IsDebugBreakOnInvalid(1)) {                                              \
    RequestDebugger();                                                         \
    bBreakOnInvalid = true;                                                    \
  }
...
static inline void DoIrqProfiling(DWORD uCycles) {
  ...
  if (g_nCycleIrqTime > g_nMax)
    g_nMax = g_nCycleIrqTime;
  if (g_nCycleIrqTime < g_nMin)
    g_nMin = g_nCycleIrqTime;
...
static inline int Fetch(BYTE &iOpcode, ULONG uExecutedCycles) {
  ...
  iOpcode = ((regs.pc & 0xF000) == 0xC000)
                ? IORead[(regs.pc >> 4) & 0xFF](
                      regs.pc, regs.pc, 0, 0,
                      uExecutedCycles) // Fetch opcode from I/O memory, but
                                       // params are still from mem[]
                : *(mem + regs.pc);
...
static inline void NMI(ULONG &uExecutedCycles, UINT &uExtraCycles, BOOL &flagc,
                       BOOL &flagn, BOOL &flagv, BOOL &flagz) {
...
static inline void IRQ(ULONG &uExecutedCycles, UINT &uExtraCycles, BOOL &flagc,
                       BOOL &flagn, BOOL &flagv, BOOL &flagz) {
...
#ifdef UPDATE_ALL_PER_CYCLE // TODO: Measure perf increase by using this new
                            // method
...
#ifndef UPDATE_ALL_PER_CYCLE
  MB_UpdateCycles(uExecutedCycles); // Update 6522s (NB. Do this before updating
                                    // g_nCumulativeCycles below)
#endif
  //
