...
static BYTE DiskControlMotor(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                             ULONG nCyclesLeft);
static BYTE DiskControlStepper(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                               ULONG nCyclesLeft);
static BYTE DiskEnable(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                       ULONG nCyclesLeft);
static BYTE DiskReadWrite(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                          ULONG nCyclesLeft);
static BYTE DiskSetLatchValue(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                              ULONG nCyclesLeft);
static BYTE DiskSetReadMode(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                            ULONG nCyclesLeft);
static BYTE DiskSetWriteMode(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                             ULONG nCyclesLeft);
...
//===========================================================================
char *GetImageTitle(LPCTSTR imagefilename,
                    Disk_t *fptr) { // returns image title
...
static void AllocTrack(int drive) {
  ...
  fptr->trackimage =
      (LPBYTE)VirtualAlloc(NULL, NIBBLES_PER_TRACK, MEM_COMMIT, PAGE_READWRITE);
}
...
//===========================================================================
LPCTSTR DiskGetFullName(int drive) { return g_aFloppyDisk[drive].fullname; }

//===========================================================================
void DiskGetLightStatus(int *pDisk1Status_, int *pDisk2Status_) {
  //  *drive1 = g_aFloppyDisk[0].spinning ? g_aFloppyDisk[0].writelight ? 2 : 1
  //  : 0;
  //  *drive2 = g_aFloppyDisk[1].spinning ? g_aFloppyDisk[1].writelight ? 2 : 1
  //  : 0;
...
//===========================================================================
LPCTSTR DiskGetName(int drive) { return g_aFloppyDisk[drive].imagename; }
...
void DiskInitialize() {
  ...
  /*  TCHAR imagefilename[MAX_PATH];
    ...
    _tcscat(imagefilename,TEXT("MASTER.DSK")); // TODO: Should remember last
    disk by user*/
...
//===========================================================================
////////////////////////// Unzip .gz file to drive0.dsk or drive1.dsk (set in
///fname) into current directory
...
//===========================================================================
////////////////////////// Unzip .zip file to drive0.dsk or drive1.dsk (set in
///fname) into current directory
...
//===========================================================================
int DiskInsert(int drive, LPCTSTR imagefilename, BOOL writeprotected,
  ...
  if (lf > 3 && imagefilename[lf - 1] == 'z' && imagefilename[lf - 2] == 'g' &&
      imagefilename[lf - 3] == '.') { // .gz?
  ...
  } else if (lf > 4 && imagefilename[lf - 1] == 'p' &&
             imagefilename[lf - 2] == 'i' && imagefilename[lf - 3] == 'z' &&
...
//===========================================================================
BOOL DiskIsSpinning() { return floppymotoron; }
...
void DiskNotifyInvalidImage(LPCTSTR imagefilename, int error) {
  ...
  case 1:
    sprintf(buffer, TEXT("Unable to open the file %s."),
...
void DiskSelectImage(int drive, LPSTR pszFilename) {
  ...
  RegSaveString(TEXT("Preferences"), REGVALUE_PREF_START_DIR, 1,
                g_sCurrentDir); // save it
  ...
  if (!error) {
    ...
    DiskNotifyInvalidImage(
        filename,
        error); // show error on the screen (or in console for our case)
  }
...
void Disk_FTP_SelectImage(int drive) // select a disk image using FTP
{
  ...
  RegSaveString(TEXT("Preferences"), REGVALUE_FTP_DIR, 1,
                g_sFTPServer); // save it

  snprintf(tmppath, MAX_PATH, "%s/%s", fullpath, filename);
  strcpy(fullpath, tmppath); // fullpath - full path to file on FTP server

  snprintf(tmppath, MAX_PATH, "%s/%s", g_sFTPLocalDir,
...
// we do not want to download it again
...
#else
  // using WIN32 method
  if (GetFileAttributes(tmppath) != DWORD(-1))
    error = 0;
  else
    error = ftp_get(fullpath, tmppath); // 0 on success
#endif
  ...
  if (!error) {
    error = DiskInsert(drive, tmppath, 0,
                       1); // try to insert downloaded file as a disk image
    if (!error) {
      /*
        if(drive == 0)
          RegSaveString(TEXT("Preferences"),REGVALUE_DISK_IMAGE1, 1, fullpath);
...
bool DiskDriveSwap() {
  ...
  // change title
  snprintf(s_title, MAX_DISK_IMAGE_NAME + 32, "%s - %s", g_pAppTitle,
           g_aFloppyDisk[0].imagename);
  SDL_WM_SetCaption(s_title,
                    g_pAppTitle); // change caption just for drive 0 (leading)
...
// static BYTE Disk_IORead(WORD pc, BYTE addr, BYTE bWrite, BYTE d, ULONG
// nCyclesLeft);
// static BYTE Disk_IOWrite(WORD pc, BYTE addr, BYTE bWrite, BYTE d, ULONG
// nCyclesLeft);

void DiskLoadRom(LPBYTE pCxRomPeripheral, UINT uSlot) {
  const UINT DISK2_FW_SIZE = 256;

  //   HRSRC hResInfo = FindResource(NULL, MAKEINTRESOURCE(IDR_DISK2_FW),
  //   "FIRMWARE");
  //   if(hResInfo == NULL)
  //     return;
  //
...
/*static*/ BYTE Disk_IORead(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                            ULONG nCyclesLeft) {
  addr &= 0xFF;
...
/*static*/ BYTE Disk_IOWrite(WORD pc, WORD addr, BYTE bWrite, BYTE d,
                             ULONG nCyclesLeft) {
  addr &= 0xFF;
...
DWORD DiskSetSnapshot(SS_CARD_DISK2 *pSS, DWORD /*dwSlot*/) {
  ...
  for (UINT i = 0; i < 2; i++) {
    ...
    //     {
    //       BOOL bWriteProtected = (/*dwAttributes & FILE_ATTRIBUTE_READONLY*/
    //       buf.st_mode & S_IWUSR) ? /*TRUE : FALSE*/FALSE : TRUE;
    //
