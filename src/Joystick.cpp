...
void CheckJoystick1() {
  ...
  if ((currtime - lastcheck >= 10) || joybutton[2]) {
    ...
    if (joyinfo[joytype[1]].device != DEVICE_NONE)
      joybutton[1] = SDL_JoystickGetButton(
          joy2, 0); // Re-map this button when emulating a 2nd Apple joystick
  ...
  if (joyinfo[joytype[0]].device == DEVICE_JOYSTICK) {
    ...
    if (number_of_joysticks >
        0) //(joyGetDevCaps(JOYSTICKID1,&caps,sizeof(JOYCAPS)) ==
           //JOYERR_NOERROR)
    {
      joy1 = SDL_JoystickOpen(
          0); // open joystick and get its information into SDL_Joystick struct
      joyshrx[0] = 0;
      joyshry[0] = 0;
  ...
  if (joyinfo[joytype[1]].device == DEVICE_JOYSTICK) {
    ...
    if (number_of_joysticks >
        1) //(joyGetDevCaps(JOYSTICKID2,&caps,sizeof(JOYCAPS)) ==
           //JOYERR_NOERROR)
    {
      joy2 = SDL_JoystickOpen(1); // open joystick #2 and get its information
                                  // into SDL_Joystick struct
      joyshrx[1] = 0;
      joyshry[1] = 0;
...
BOOL JoySetEmulationType(/*HWND window,*/ DWORD newtype, int nJoystickNumber) {
  ...
  if (joyinfo[newtype].device == DEVICE_JOYSTICK) {
    ...
    if (sg_Mouse.Active()) {
      /*MessageBox(window,
           TEXT("Mouse interface card is enabled - unable to use mouse for
         joystick emulation."),
           TEXT("Configuration"),
           MB_ICONEXCLAMATION | MB_SETFOREGROUND);*/
