...
bool DiskFTPChooseAnImage(char *ftp_dir, int slot, char **name, bool *isdir,
	...
	snprintf(ftpdirpath, MAX_PATH, "%s/%s%s", g_sFTPLocalDir,
		 g_sFTPDirListing,
	...
	bool OKI;
	if (stat(ftpdirpath, &info) == 0 &&
	    info.st_mtime > time(NULL) - RENEW_TIME) {
	...
	if (OKI) { // error
		printf("Failed getting FTP directory %s to %s\n", ftp_dir,
		       ftpdirpath);
		font_print_centered(g_ScreenWidth / 2, 30 * facy,
				    "Failure! Press any key.", screen, fs, fs);
		SDL_Flip(screen);     // show the screen
	...
	while (
	    (tmp = fgets(tmpstr, 512, fdir))) // first looking for directories
	{
		...
		{
			tmp = new char[strlen(FTP_PARSE.name) +
				       1]; // add entity to list
	...
	while (true) {
		SDL_BlitSurface(my_screen, NULL, screen,
		...
		while (files.Iterate(tmp)) {
			...
			if (i >= first_file &&
				...
				if (i == act_file) {
					...
					SDL_Rect r;
					r.x = 0;
					/* highlight is 4 pixels taller than
					 * text */
					r.y = y - 2;
					r.w = g_ScreenWidth;
					/* highlight is 4 pixels taller than
		...
		// control cursor
		keyboard = SDL_GetKeyState(NULL); // get current state of
		...
		if (keyboard[SDLK_RETURN]) {
			...
			*name = strdup(
			    php_trim(files[act_file], strlen(files[act_file])));
			//      printf("files[act_file]=%s, *name=%s\n\n",
			//      files[act_file],
			//      *name);
			if (!strcmp(sizes[act_file], "<DIR>") ||
			    !strcmp(sizes[act_file], "<UP>"))
				*isdir = true;
			else
				*isdir = false; // this is directory (catalog in
						// Apple][ terminology)
		...
		if (keyboard[SDLK_END]) { // END?
			act_file = files.Length() -
				   1; // go to the last possible file in list
			first_file = act_file - FILES_IN_SCREEN + 1;
			if (first_file < 0)
				first_file = 0;
		} /* if */
