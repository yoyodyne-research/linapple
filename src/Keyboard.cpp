...
#else
// Buffered key input:
// - Needed on faster PCs where aliasing occurs during short/fast bursts of 6502
// code.
// - Keyboard only sampled during 6502 execution, so if it's run too fast then
...
void KeybReset() {
  ...
  g_nKeyBufferSize =
      g_bKeybBufferEnable ? KEY_BUFFER_MAX_SIZE : KEY_BUFFER_MIN_SIZE;
#endif
}
...
void KeybUpdateCtrlShiftStatus() {
  //  g_bShiftKey = (GetKeyState( VK_SHIFT  ) & KF_UP) ? true : false; // 0x8000
  //  KF_UP
...
void KeybQueueKeypress(int key, BOOL bASCII) {
  ...
  if (bASCII == ASCII) {
    ...
    // may be this is straitfoward method, but it seems to be working. What else
    // we need?? --bb
    ...
    /*    if ((key == VK_CANCEL) && (GetKeyState(VK_CONTROL) < 0)) - implement
    in Frame.cpp
        {
          // Ctrl+Reset
          if (!IS_APPLE2)
    ...
    */
    ...
    //     if (!((key >= VK_LEFT) && (key <= VK_DELETE) && asciicode[IS_APPLE2 ?
    //     0 : 1][key - VK_LEFT]))
    //       return;
    //     keycode = asciicode[IS_APPLE2 ? 0 : 1][key - VK_LEFT];    // Convert
    //     to Apple arrow keycode
    //     lastvirtkey = key;
    //     {0x08,0x0D,0x15,0x2F,0x00,0x00,0x00,0x00,0x00,0x00}, - good old
    //     APPLE2
    //     {0x08,0x0B,0x15,0x0A,0x00,0x00,0x00,0x00,0x00,0x7F}
...
BYTE /*__stdcall */ KeybReadFlag(WORD, WORD, BYTE, BYTE, ULONG) {
  ...
  Uint8 *keys;
  keys = SDL_GetKeyState(NULL); // get current key state - thanx to SDL
                                // developers! ^_^ beom beotiger
#ifdef KEY_OLD
  keywaiting = 0;
...
void KeybToggleCapsLock() {
  if (!IS_APPLE2) {
    g_bCapsLock = !g_bCapsLock; // never mind real CapsLock status,
                                // heh???(GetKeyState(VK_CAPITAL) & 1);
