...
void Config::ChangeToHomeDirectory() {
  if (chdir(GetHomePath().c_str())) {
    // TODO: LOG ERROR
    cout << "Cannot switch to home directory ('" << GetHomePath().c_str()
         << "')" << std::endl;
  }
}
...
void Config::ChangeToUserDirectory() {
  if (chdir(GetUserFilePath().c_str())) {
    ...
    cout << "Cannot switch to user directory ('" << GetUserFilePath().c_str()
         << "')" << std::endl;
  }
}
...
bool Config::CopyFile(std::string srcFile, std::string destFile) {
  ...
  std::cout << "Copying '" << srcFile.c_str() << "' to '" << destFile.c_str()
            << "'" << std::endl;
  ...
  if (source && dest) {
    ...
    while ((size = read(source, buf, bufSize)) > 0) {
      if (0 >= write(dest, buf, size)) {
        // Handle error;
        std::cout << "Error writing '" << destFile.c_str() << "' (" << size
                  << ")" << std::endl;
