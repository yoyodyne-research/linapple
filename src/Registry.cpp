...
BOOL ReturnKeyValue(char *line, char **key, char **value) {
  ...
  *key = php_trim(
      line,
      strlen(line)); // trim those strings from beginning and trailing spaces
  if (*key != NULL && **key == '#')
...
char *ReadRegString(char *key) {
  // reads key for given value from the registry. Hmmm. What registry in Linux?
  // I donna. --bb
...
//===========================================================================
BOOL RegLoadString(LPCTSTR section, LPCTSTR key, BOOL peruser, char **buffer,
                   DWORD chars) {
