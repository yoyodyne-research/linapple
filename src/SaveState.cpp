...
char *Snapshot_GetFilename() { return g_szSaveStateFilename; }
...
void Snapshot_LoadState() {
  ...
  APPLEWIN_SNAPSHOT *pSS =
      (APPLEWIN_SNAPSHOT *)new char[sizeof(APPLEWIN_SNAPSHOT)];
  ...
  try {
    ...
    DWORD dwBytesRead;
    BOOL bRes =
        ReadFile(hFile, pSS, sizeof(APPLEWIN_SNAPSHOT), &dwBytesRead, NULL);
...
void Snapshot_SaveState() {
  ...
  if (hFile != INVALID_HANDLE_VALUE) {
    ...
    /*BOOL bRes =*/WriteFile(hFile, pSS, sizeof(APPLEWIN_SNAPSHOT),
                             &dwBytesWritten, NULL);
